...
export const pushUrlQuery = curry((push: Function, query: string) => {
  ...
  const { inputValue, dropdownValue } = query;

  push({
    pathname: '/search',
    search: `per_page=${PER_PAGE}&page=1&q=${inputValue}+language:${dropdownValue}`,
  });
});
...
const renderRoute = curry((Component, matchProps) => {
  ...
  return (
    <div className={`${css(styles.DefaultLayout)} u-flex u-flexCol`}>
      <HeaderContainer searchTerm={searchTerm} searchQuery={qs.stringify(parsedSearch)} onSubmit={pushUrlQuery(push)} />
      ...
      <Container rootStyle={styles.DefaultLayout_footer}>
        <div className="u-flex u-flexWrap u-flexAlignItemsCenter u-flexJustifyBetween">
          <p className={css(styles.DefaultLayout_footerText)}>
            View the <a href="https://github.com/rupeshdabbir/github-repo-search-project">source on GitHub</a>
          </p>
          <RateLimitContainer />
        </div>
      </Container>
